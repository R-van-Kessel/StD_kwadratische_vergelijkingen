...
function openRekenkaart() { 

    window.open('https://r-van-kessel.github.io/Summon_the_Dragon/rekenkaart.html', '_blank'); 
    ...
    popup.innerHTML = ` 
        ...
        <p style="color: #0E0E0E; line-height: 1.2;"> 
            ...
            <ol style="color: #0909B4; margin: 5px 0;"> 
                ...
                <li>Als alle antwoorden goed zijn start de Dragon game automatisch! </li> 

                <li>Klik op "Rekenkaart" want die kan je helpen bij het oplossen van veel sommen.</li> 

                <li>Klik op "Reset" voor 10 nieuwe sommen</li> 
...
function generateQuestions() {
    ...
    function answerText(p, q) {
        if (p === null || q === null) return `x = ${p ?? q}`;
        if (p === q) return `x = ${p}`;
        let vals = [p, q].sort((a, b) => b - a);
        return `x = ${vals[0]} of\nx = ${vals[1]}`;
    }

    const types = [
        // Type 1: ax² + c = d  →  x = ±√((d-c)/a)
        () => {
            let a = floor(random(1, 6));
            let x = floor(random(1, 8));
            let rhs = a * x * x;
            let c = floor(random(-20, 20));
            if (c === 0) c = 1;
            let d = rhs + c;
            let cStr = c > 0 ? `+ ${c}` : `- ${Math.abs(c)}`;
            return {
                text: `${coef(a)}x² ${cStr} = ${d}`,
                answer: x,
                answer2: -x
            };
        },

        // Type 2: ax² = bx  →  x=0 of x=b/a
        () => {
            let a = floor(random(2, 6));
            let x = floor(random(2, 8));
            let b = a * x;
            return {
                text: `${coef(a)}x² = ${b}x`,
                answer: x,
        ...
        // Type 3: x² = bx + c  →  (x-p)(x+q)=0
        () => {
            let p = floor(random(2, 8));
            let q = floor(random(1, 6));
            let bCoef = p - q;
            let cCoef = p * q;

            // Coëfficiënt voor x zonder 1 of -1
            let bStr;
            if (bCoef === 0)       bStr = '';
            else if (bCoef === 1)  bStr = 'x';
            else if (bCoef === -1) bStr = '- x';
            else if (bCoef > 0)    bStr = `${bCoef}x`;
            else                   bStr = `- ${Math.abs(bCoef)}x`;

            let cStr = cCoef !== 0 ? (cCoef > 0 ? `+ ${cCoef}` : `- ${Math.abs(cCoef)}`) : '';

            let text;
            if (bStr && cStr)      text = `x² = ${bStr} ${cStr}`;
            else if (bStr)         text = `x² = ${bStr}`;
            else                   text = `x² = ${cCoef}`;

            return { text, answer: p, answer2: -q };
        },

        // Type 4: a·x(bx + c) = 0  →  x=0 of x=-c/b
        () => {
            let b = floor(random(2, 5));
            let x = floor(random(1, 7));
            let c = -(b * x);
            let cStr = c > 0 ? `+ ${c}` : `- ${Math.abs(c)}`;
            let a = floor(random(1, 4));
            let aStr = a === 1 ? '' : a;
            return {
                text: `${aStr}x(${b}x ${cStr}) = 0`,
                answer: x,
                answer2: 0
            };
        },

        // Type 5: (x - r1)(x - r2) = 0  →  wortels nooit 0
        () => {
            let r1 = floor(random(-6, 7));
            while (r1 === 0) r1 = floor(random(-6, 7));
            let r2 = floor(random(-6, 7));
            while (r2 === r1 || r2 === 0) r2 = floor(random(-6, 7));

            function termStr(r) {
                // r is de wortel, dus factor is (x - r)
                if (r > 0) return `x - ${r}`;
                return `x + ${Math.abs(r)}`;
            }

            return {
                text: `(${termStr(r1)})(${termStr(r2)}) = 0`,
                answer: r1,
                answer2: r2
            };
        },

        // Type 6: a(x-p)(x-q) uitgeschreven  →  ax² + bx + c = 0
        () => {
            let p = floor(random(1, 6));
            let q = floor(random(-5, 0));
            let a = floor(random(1, 4));
            let bCoef = a * (-(p + q));
            let cCoef = a * p * q;
            ...
            let cStr;
            if (cCoef === 0)       cStr = '';
            else if (cCoef > 0)    cStr = `+ ${cCoef}`;
            else                   cStr = `- ${Math.abs(cCoef)}`;

            let mid = bStr ? ` ${bStr}` : '';
            let end = cStr ? ` ${cStr}` : '';

            return {
                text: `${coef(a)}x²${mid}${end} = 0`,
                answer: p,
                answer2: q
            };
        },

        // Type 7: ax² + bx = 0  →  x=0 of x=-b/a
        () => {
            let a = floor(random(2, 5));
            let x = floor(random(2, 8));
            let b = -(a * x);
            let bStr = b > 0 ? `+ ${b}x` : `- ${Math.abs(b)}x`;
            return {
                text: `${coef(a)}x² ${bStr} = 0`,
                answer: x,
                answer2: 0
            };
        },
// Type 8: (x + p)(x + q) = c  →  x² + (p+q)x + pq - c = 0
        () => {
            // Kies twee wortels r1 en r2
            let r1 = floor(random(-6, 7));
            while (r1 === 0) r1 = floor(random(-6, 7));
            let r2 = floor(random(-6, 7));
            while (r2 === r1 || r2 === 0) r2 = floor(random(-6, 7));
            ...
            // We kiezen een willekeurige c zodat de vergelijking er interessant uitziet
            // en verschuiven: (x - r1)(x - r2) = c
            // waarbij c = een willekeurig getal ≠ 0
            let c = floor(random(-15, 15));
            while (c === 0) c = floor(random(-15, 15));

            // De echte vergelijking die opgelost moet worden:
            // x² - (r1+r2)x + r1*r2 - c = 0
            // Controleer of r1 en r2 nog steeds de wortels zijn:
            // (r1 - r1)(r1 - r2) = 0 ≠ c  dus r1 en r2 zijn GEEN wortels meer
            // We moeten nieuwe wortels berekenen via de abc-formule
            // Kies daarom anders: stel de wortels vast en bereken c achteraf

            // (x - r1)(x - r2) + c = (x - s1)(x - s2)
            // Kies s1 en s2 als echte wortels, bereken dan c = s1*s2 - r1*r2
            // en controleer of de som van wortels klopt: s1+s2 = r1+r2

            // Eenvoudiger: kies p en q als de tekens in de haakjes
            // (x + p)(x + q) = c  met bekende wortels
            // Wortels zijn x = r1 en x = r2 als:
            // (r1 + p)(r1 + q) = c  EN  (r2 + p)(r2 + q) = c
            // Kies p, q vrij en bereken c via één wortel, check de andere

            // SIMPELSTE AANPAK:
            // Kies p en q (de getallen in de haakjes)
            // Kies wortel r1, bereken c = (r1+p)(r1+q)
            // Los op: x² + (p+q)x + pq = c  →  x² + (p+q)x + (pq-c) = 0
            // Discriminant: D = (p+q)² - 4(pq-c)
            // Kies zodanig dat D een kwadraat is en wortels geheel zijn

            let p = floor(random(-5, 6));
            while (p === 0) p = floor(random(-5, 6));
            let q = floor(random(-5, 6));
            while (q === 0 || q === p) q = floor(random(-5, 6));

            // Kies wortel r1 en bereken c
            let r1new = floor(random(-5, 6));
            while (r1new === 0) r1new = floor(random(-5, 6));
            let cNew = (r1new + p) * (r1new + q);

            // Tweede wortel: x² + (p+q)x + (pq - cNew) = 0
            // Som van wortels = -(p+q), product = pq - cNew
            // r1 + r2 = -(p+q)  →  r2 = -(p+q) - r1
            let r2new = -(p + q) - r1new;

            // Controleer of r2 geheel en geldig is
            if (!Number.isInteger(r2new) || r2new === r1new || r2new === 0) return null;
            if (Math.abs(r2new) > 20 || Math.abs(r1new) > 20) return null;
            if (cNew === 0) return null;

            // Schrijf de haakjes: (x + p) waarbij p positief: x + p, negatief: x - |p|
            function bracket(val) {
                if (val > 0) return `x + ${val}`;
                return `x - ${Math.abs(val)}`;
            }

            return {
                text: `(${bracket(p)})(${bracket(q)})= ${cNew}`,
                answer: r1new, 
                answer2: r2new
            };
        },
    ...
    while (questions.length < 10 && attempts < 300) {
        attempts++;
        let typeFn = random(types);
        let q = typeFn();

        // null check toegevoegd voor types die null kunnen teruggeven
        if (!q) continue;

        let a1 = q.answer;
        let a2 = q.answer2 !== undefined ? q.answer2 : null;

        if (
            q &&
            !isNaN(a1) && isFinite(a1) && Number.isInteger(a1) &&
            (a2 === null || (isFinite(a2) && Number.isInteger(a2))) &&
            Math.abs(a1) <= 20 &&
            (a2 === null || Math.abs(a2) <= 20)
        ) {
            let key = [a1, a2].sort((a, b) => a - b).join(',');
            if (!usedAnswers.has(key)) {
                usedAnswers.add(key);
                questions.push(q);
                answers.push({ main: a1, second: a2 });
            }
        }
    }
    ...
    while (questions.length < 10) {
        let x = questions.length + 2;
        let key = `-${x},${x}`;
        if (!usedAnswers.has(key)) {
            usedAnswers.add(key);
            questions.push({ text: `x² = ${x * x}`, answer: x, answer2: -x });
            answers.push({ main: x, second: -x });
        }
    }

    answers = shuffle(answers);

    // ===== VRAAGBLOKKEN rijen 1 en 2 =====
    let questionIndex = 0;
    for (let row = 1; row < 3; row++) {
        for (let col = 0; col < COLS; col++) {
            ...
            blocks.push({
                ...
                text: q.text,
                answer: q.answer,
                answer2: q.answer2 !== undefined ? q.answer2 : null,
                isQuestion: true, isCorrect: null,
    ...
    // ===== ANTWOORDBLOKKEN rijen 3 en 4 =====
    let answerIndex = 0;
    for (let row = 3; row < 5; row++) {
        for (let col = 0; col < COLS; col++) {
            let ans = answers[answerIndex];
            let displayText = answerText(ans.main, ans.second);
            blocks.push({
                col, row,
                startCol: col, startRow: row,
                x: MARGIN + col * CELL_SIZE,
                ...
                answer: ans.main,
                answer2: ans.second,
                isQuestion: false, isCorrect: null,
                isHovered: false, hoverProgress: 0,
            });
            answerIndex++;
        }
...
function checkAnswers() {
    ...
    for (let questionBlock of blocks) {
        if (!questionBlock.isQuestion) continue;

        // Zoek antwoordblok op dezelfde cel
        ...
        if (answerBlock) {
            let q1 = questionBlock.answer;
            let q2 = questionBlock.answer2 !== undefined ? questionBlock.answer2 : null;
            let a1 = answerBlock.answer;
            let a2 = answerBlock.answer2 !== undefined ? answerBlock.answer2 : null;

            // Correct als beide oplossingen overeenkomen (volgorde maakt niet uit)
            let correct =
                (q1 === a1 && q2 === a2) ||
                (q1 === a2 && q2 === a1);

            questionBlock.isCorrect = correct;
            answerBlock.isCorrect = correct;

            if (correct) correctCount++;
        } else {
            // Geen antwoordblok geplaatst → fout
            questionBlock.isCorrect = false;
        }
    }

    // Score button bijwerken
